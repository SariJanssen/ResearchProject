...
#pragma region COMPOSITES
	...
	//--- PARALLEL ---
	class BehaviorParallelNode : public BehaviorComposite
	{
	public:
		enum class Policy
		{
		...
		explicit BehaviorParallelNode(std::vector<IBehavior*> childrenBehaviors, Policy success, Policy failure)
			: BehaviorComposite(childrenBehaviors), m_SuccessPolicy{ success }, m_FailurePolicy{failure} {}
		virtual ~BehaviorParallelNode() = default;
...
#pragma endregion
	...
	{
	public:
		explicit BehaviorInverter(std::function<bool(Blackboard*)> fp) : m_fpBehavior(fp) {}
		virtual BehaviorState Execute(Blackboard* pBlackBoard) override;

	private:
		std::function<bool(Blackboard*)> m_fpBehavior = nullptr;
	};
