	...
	public:
		...
		static std::vector<Elite::Vector2> OptimizePortals(const std::vector<Portal>& portals)
		{
			...
			for (unsigned int i = 1; i < static_cast<unsigned int>(portals.size()); ++i)
			{
				...
				if (Cross(rightLeg, newRightLeg) >= 0.f) // check if going inwards
				{
					if (Cross(leftLeg, newRightLeg) <= 0.f) // check if crossing left leg
					{
				...
				if (Cross(leftLeg, newLeftLeg) <= 0.f) // check if going inwards
				{
					if (Cross(rightLeg, newLeftLeg) >= 0.f) // check if crossing left leg
					{
						leftLeg = newLeftLeg;
						leftLegIndex = i;
					}
